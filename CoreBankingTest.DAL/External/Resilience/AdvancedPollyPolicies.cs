...
    public IAsyncPolicy<HttpResponseMessage> CreateHttpResiliencePipeline() 
    ...
    {        
        var jitterer = new Random();
        var retryPolicy = Policy<T>.Handle<Exception>()
        ...
        var circuitBreaker = Policy<T>.Handle<Exception>().
...
    private IAsyncPolicy<HttpResponseMessage> CreateHttpCircuitBreakerPolicy() 
    { 
        return Policy<HttpResponseMessage>.Handle<HttpRequestException>().OrResult(r => (int)r.StatusCode >= 500).CircuitBreakerAsync(3, TimeSpan.FromSeconds(30)); 
    }
}